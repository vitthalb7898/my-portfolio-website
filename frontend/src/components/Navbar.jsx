          ...
          <nav className={cn("fixed w-full z-40 transition-all duration-300" , 
            ...
            <div className="container flex items-center justify-between">
                ...
                <div className="hidden md:flex space-x-10">
                    {navItems.map((item) => (
                        <a key={item.name} className="text-foreground/80 hover:text-primary transition-colors duration-300" href={item.href}>
                            {item.name}
                        </a>
                    ))}
                </div>
                ...
                <div className={cn("fixed inset-0 bg-background/95 backdrop-blur-md z-40 flex flex-col items-center justify-center", "transition-all duration-300 md:hidden",
                    ...
                    {navItems.map((item) => (
                        <a key={item.name} className="text-foreground/80 hover:text-primary transition-colors duration-300" href={item.href} onClick={() => setIsMenuOpen(false)}>
                            {item.name}
                        </a>
                    ))}
                </div>
